...
class GenericPlot(app.Canvas):
    def __init__(self, plot_mode: str = "windowed"):
        app.use_app("PySide6")
        app.Canvas.__init__(
            self, title="Use your wheel to zoom!", keys="interactive", app="PySide6"
        )
    ...
    def initiateCanvas(
        self, color, index, nrows=1, ncols=1, plot_window_sample_count=10000
    ):
        ...
        if color is None:
            color = np.repeat(
                np.random.uniform(size=(self.m, 3), low=0.5, high=0.9), self.n, axis=0
            ).astype(np.float32)

        if index is None:
            index = np.c_[
                np.repeat(np.repeat(np.arange(self.ncols), self.nrows), self.n),
    ...
    def plot_new_data(self, data_frame, next_val):
        ...
        emgLen = max(
            len(x) for x in data_frame
        )  # All processing is normalized to the fastest EMG rate
            ...
            if len(data_frame[i]) < emgLen:
                ...
                for j in range(len(quantIndexVector)):
                    if j == len(quantIndexVector) - 1:
                        ...
                        arrToInsert = np.linspace(
                            quantData[quantIndexVector[j]],
                            next_val[i],
                            len(quantData) - quantIndexVector[j],
                            endpoint=True,
                        )
                        ...
                        arrToInsert = np.linspace(
                            quantData[quantIndexVector[j]],
                            quantData[quantIndexVector[j + 1]],
                            quantIndexVector[j + 1] - quantIndexVector[j],
                            endpoint=False,
                        )
                        quantData[quantIndexVector[j] : quantIndexVector[j + 1]] = (
                            arrToInsert
                        )
